...
use app\common\library\AesCrypt;
...
{
    protected $noNeedLogin = ['init', 'captcha','handleEncryptedData'];
    ...
    public function _initialize()
    {
        ...
        $key = config('aes_key'); // 256 位密钥
        $iv = config('aes_iv');  // 16 字节的 IV
     ...
     * @return \think\Response
     */
    public function handleEncryptedData()
    {
        $encryptedData = $this->request->post('encrypted');
        if (!$encryptedData) {
            return json(['code' => 400, 'message' => '缺少加密数据']);
        }
        ...
        try {
            ...
            if ($jsonData === null) {
                return json(['code' => 400, 'message' => '解密后的数据不是有效的 JSON']);
            }
            return json(['code' => 200, 'message' => '处理成功', 'data' => $jsonData]);
        } catch (\Exception $e) {
            return json(['code' => 500, 'message' => '解密失败：' . $e->getMessage()]);
        }
    }
}
